...
# Usage:
# /applications/R/R-4.0.0/bin/Rscript CEN180_CENgap_CENAthila_nonCENAthila_nonCENGypsy_CENsoloLTR_RNAseq_7metaprofiles.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 180 2000 2000 2000 '2kb' 10 10 10bp 10bp '0.02,0.96' 'WT_RNAseq_Rep1,met1_RNAseq_Rep1,Col_0_RNAseq_Rep1_SRR11780903,ddm1_RNAseq_Rep1_SRR11780906,Col_0_RNAseq_Rep1_SRR1005385,drm1_drm2_cmt2_cmt3_RNAseq_Rep1_SRR1005399,kss_RNAseq_Rep1_SRR1005401' '160601_Kyuha_RNAseq/snakemake_RNAseq_STAR_t2t-col.20210610,160601_Kyuha_RNAseq/snakemake_RNAseq_STAR_t2t-col.20210610,RNAseq_leaf_Osakabe_Berger_2021_NCB/snakemake_RNAseq_STAR_t2t-col.20210610,RNAseq_leaf_Osakabe_Berger_2021_NCB/snakemake_RNAseq_STAR_t2t-col.20210610,RNAseq_leaf_Stroud_Jacobsen_2014_NSMB/snakemake_RNAseq_STAR_t2t-col.20210610,RNAseq_leaf_Stroud_Jacobsen_2014_NSMB/snakemake_RNAseq_STAR_t2t-col.20210610,RNAseq_leaf_Stroud_Jacobsen_2014_NSMB/snakemake_RNAseq_STAR_t2t-col.20210610' 'WT (Choi et al. 2018),met1-3 (Choi et al. 2018),WT (Osakabe et al. 2021),ddm1 (Osakabe et al. 2021),WT (Stroud et al. 2014),ddcc (Stroud et al. 2014),kss (Stroud et al. 2014)' 'springgreen2,magenta,forestgreen,purple4,darkgreen,blue,deepskyblue'
...
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("160601_Kyuha_RNAseq/snakemake_RNAseq_STAR_t2t-col.20210610,160601_Kyuha_RNAseq/snakemake_RNAseq_STAR_t2t-col.20210610",
#                                split = ","))
...
ChIP_featureMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_", align, "_sort_norm_CEN180_in_",
...
ChIP_gapMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_", align, "_sort_norm_CENgap_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_TEsfMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "TEprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_", align, "_sort_norm_nonCEN_TEs_Gypsy_LTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_soloLTRMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CENsoloLTRprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_", align, "_sort_norm_CENsoloLTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ggsave(paste0(plotDir,
              ...
              "_avgProfiles_around",
              "_CEN180_ranLoc_CENgap_nonCENAthila_CENAthila_nonCENGypsy_CENsoloLTR_in_t2t-col.20210610_",
