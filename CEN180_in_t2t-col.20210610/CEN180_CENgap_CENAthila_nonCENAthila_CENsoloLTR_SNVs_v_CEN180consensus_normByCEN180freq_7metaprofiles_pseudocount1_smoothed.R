...
# Usage:
# /applications/R/R-4.0.0/bin/Rscript CEN180_CENgap_CENAthila_nonCENAthila_CENsoloLTR_SNVs_v_CEN180consensus_normByCEN180freq_7metaprofiles_pseudocount1_smoothed.R 'Chr1,Chr2,Chr3,Chr4,Chr5' 180 2000 2000 2kb 10 10 10bp 10bp '0.02,0.96'
...
ChIP_nonCENAthilaMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_nonCENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
...
control_nonCENAthilaMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_nonCENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
...
# nonCENAthila
ChIPcontrol_nonCENAthilaMats <- mclapply(seq_along(ChIP_nonCENAthilaMats), function(x) {
  cbind( ( ( ChIP_nonCENAthilaMats[[x]][,1:(upstream/binSize)] * binSize ) + 1 ) / ( control_nonCENAthilaMats[[1]][,1:(upstream/binSize)] + 1 ),
         ChIP_nonCENAthilaMats[[x]][,((upstream/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize))],
         ( ( ChIP_nonCENAthilaMats[[x]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] * binSize ) + 1 ) /
             ( control_nonCENAthilaMats[[1]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] + 1 )
       )
}, mc.cores = length(ChIP_nonCENAthilaMats))
# soloLTR
...
for(x in seq_along(ChIPcontrol_featureMats)) {
  ...
  colnames(ChIPcontrol_AthilaMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                             paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                             paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
  colnames(ChIPcontrol_nonCENAthilaMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                                   paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                                   paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
ChIPcontrol_mats <- mclapply(seq_along(ChIPcontrol_featureMats), function(x) {
  list(
       ...
       # nonCENAthilas
       ChIPcontrol_nonCENAthilaMats[[x]],
       # soloLTRs
...
nonCENAthilaTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
...
#              paste0(ChIPNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgap_CENAthila_nonCENAthila_CENsoloLTR_in_t2t-col.20210610_",
