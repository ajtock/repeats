...
# Usage:
# CEN180_CENgapAll_CENgap_CENAthila_CENsoloLTR_SNVs_v_CEN180consensus_normByCEN180freq_7metaprofiles_pseudocount1_smoothed.R 'Chr1,Chr2,Chr3,Chr4,Chr5' 180 2000 2000 2kb 10 10 10bp 10bp '0.02,0.96'
...
ChIP_gapMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENgap_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
...
control_gapMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENgap_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
...
#              paste0(ChIPNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgapAll_CENgap_CENAthila_CENsoloLTR_in_t2t-col.20210610_",
