...
# Usage:
# /applications/R/R-4.0.0/bin/Rscript CEN180_CENAthila_nonCENAthila_nonCENGypsy_5metaprofiles.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 180 2000 2000 2000 '2kb' 10 10 10bp 10bp '0.02,0.96' 'DMC1_V5_Ler_F1,DMC1_V5_Rep2,AtDMC1_colcviF1_ChIP,DMC1_V5_CMT3_Rep1,DMC1_V5_KSS_Rep1' 'ColCvi/snakemake_ChIPseq_t2t-col.20210610,ColCvi/snakemake_ChIPseq_t2t-col.20210610,ColCvi/snakemake_ChIPseq_t2t-col.20210610,ColCvi/snakemake_ChIPseq_t2t-col.20210610,ColCvi/snakemake_ChIPseq_t2t-col.20210610' 'ColxLer F1 DMC1,DMC1 WT,ColxCvi F1 DMC1,cmt3 DMC1,kss DMC1' 'darkorange1,dodgerblue1,red,purple,green' 'ChIP'
...
ChIPDirs <- sapply(seq_along(ChIPNames), function(x) {
  paste0("/production/dh580/novogene/",
         ChIPNamesDir[x],
         "/mapped/")
})
...
log2ChIP_featureMats <- mclapply(seq_along(ChIP_featureMats), function(x) {
  if ( grepl("MNase", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[4], " for log2((MNase+1)/(gDNA+1)) calculation"))
    log2((ChIP_featureMats[[x]]+1)/(control_featureMats[[4]]+1))
  } else if ( grepl("SPO11oligos", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[5], " for log2((SPO11-1-oligos+1)/(gDNA+1)) calculation"))
    log2((ChIP_featureMats[[x]]+1)/(control_featureMats[[5]]+1))
  } else if ( grepl("SRR44305", ChIPNames[x]) ) {
  ...
  } else if ( grepl("H3K9me2", ChIPNames[x]) ) {
