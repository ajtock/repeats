...
CEN180[,2] <- CEN180[,2]+1
colnames(CEN180) <- c("chr", "start", "end", "featureID", "wSNV", "strand", "HORlengthsSum", "HORcount", "percentageIdentity")
...
DNAmeth_featureMats <- mclapply(seq_along(DNAmethContexts), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(DNAmethDirs[x],
                                DNAmethNames[x],
                                "_MappedOn_t2t-col.20210610_", DNAmethContexts[x], "_CEN180_in_",
...
ChIP_featureMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_lowXM_both_sort_norm_CEN180_in_",
                                chrName[y], "_matrix_bin", binName, "_flank", flankName, ".tab"),
                         header = F, skip = 3,
                         colClasses = c(rep("NULL", 1000/binSize), rep(NA, ((1000*2)+(180))/binSize), rep("NULL", 1000/binSize))))
  })
...
log2ChIP_featureMats <- mclapply(seq_along(ChIP_featureMats), function(x) {
  if ( grepl("CENH3", ChIPNames[x]) ) {
    ...
    print(paste0(ChIPNames[x], " library; using ", controlNames[1], " for log2((MNase+1)/(input+1)) calculation"))
    log2((ChIP_featureMats[[x]]+1)/(control_featureMats[[1]]+1))
  } else if ( grepl("SPO11oligos", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[1], " for log2((SPO11-1-oligos+1)/(input+1)) calculation"))
    log2((ChIP_featureMats[[x]]+1)/(control_featureMats[[1]]+1))
  } else {
...
CEN180 <- data.frame(CEN180,
                     ...
                     LoCENH3_in_bodies = log2ChIP_featureMats_bodiesRowMeans[[2]],
                     ZmCENH3_in_bodies = log2ChIP_featureMats_bodiesRowMeans[[3]],
                     H3K9me2_in_bodies = log2ChIP_featureMats_bodiesRowMeans[[4]],
...
ranLoc <- data.frame(ranLoc,
                     ...
                     LoCENH3_in_bodies = log2ChIP_ranLocMats_bodiesRowMeans[[2]],
                     ZmCENH3_in_bodies = log2ChIP_ranLocMats_bodiesRowMeans[[3]],
                     H3K9me2_in_bodies = log2ChIP_ranLocMats_bodiesRowMeans[[4]],
