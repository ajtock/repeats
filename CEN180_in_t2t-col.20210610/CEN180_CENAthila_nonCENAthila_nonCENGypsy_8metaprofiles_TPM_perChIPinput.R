...
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("CENH3_seedlings_Maheshwari_Comai_2017_GenomeRes/snakemake_ChIPseq_t2t-col.20210610,170101_Chris_H3K9me2_ChIP/WT/snakemake_ChIPseq_t2t-col.20210610",
#                                split = ","))
#ChIPNamesPlot <- unlist(strsplit("CENH3,H3K9me2",
#                                 split = ","))
#ChIPColours <- unlist(strsplit("darkorange1,dodgerblue1",
#                               split = ","))
#yLabPlot <- "ChIP"
...
ChIP_featureMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_lowXM_", align, "_sort_TPM_CEN180_in_",
...
ChIP_AthilaMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CENAthilaProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_lowXM_", align, "_sort_TPM_CENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_nonCENAthilaMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CENAthilaProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_lowXM_", align, "_sort_TPM_nonCENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_TEsfMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "TEprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_t2t-col.20210610_lowXM_", align, "_sort_TPM_nonCEN_TEs_Gypsy_LTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ggsave(paste0(plotDir,
              ...
              paste0(chrName, collapse = "_"), "_", align, "_TPM_perChIPinput.pdf"),
...
ggsave(paste0(plotDir,
              ...
              "_avgProfiles_around",
              "_CEN180_ranLoc_CENAthila_nonCENAthila_nonCENGypsy_in_t2t-col.20210610_",
              paste0(chrName, collapse = "_"), "_", align, "_TPM.pdf"),
       plot = ggObjGA_combined,
       height = 6.5, width = 7*5, limitsize = FALSE)
