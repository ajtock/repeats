...
# Usage:
# CEN180_CENgapSNVT_CENgapSNVF_CENAthila_CENsoloLTR_SNVs_v_CEN180consensus_normByCEN180freq_7metaprofiles_pseudocount1_smoothed.R 'Chr1,Chr2,Chr3,Chr4,Chr5' 180 2000 2000 2kb 10 10 10bp 10bp '0.02,0.96'
...
ChIPNames <- c("all",
               ...
               "transition",
               "transversion")
ChIPDirs <- rep("/home/ajt200/analysis/nanopore/t2t-col.20210610/SNVs_v_CEN180consensus/CEN180profiles/matrices_smoothed/",
                length(ChIPNames))
ChIPNamesPlot <- c("All",
                   "SNVs",
                   "Indels",
...
controlDirs <- rep("/home/ajt200/analysis/nanopore/t2t-col.20210610/SNVs_v_CEN180consensus/CEN180profiles/matrices_smoothed/",
                   length(controlNames))
controlNamesPlot <- c("Features")
controlColours <- c("red")
yLabPlot <- "Norm. CEN180 variants"
yLabPlotVariants <- "CEN180 variants"
yLabPlotCEN180 <- "CEN180"
...
if(length(chrName) == 5) {
  ...
  gapSNVTNamePlot <- "All CENgapSNVT"
  gapSNVFNamePlot <- "All CENgapSNVF"
  ...
  gapSNVTNamePlot <- paste0(paste0(chrName, collapse = ","), " CENgapSNVT")
  gapSNVFNamePlot <- paste0(paste0(chrName, collapse = ","), " CENgapSNVF")
...
ChIP_featureMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CEN180_in_",
...
## ChIP
# gapSNVT
ChIP_gapSNVTMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENgapSNVT_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
# If gapSNVTs from multiple chromosomes are to be analysed,
# concatenate the corresponding gapSNVT coverage matrices
ChIP_gapSNVTMats <- mclapply(seq_along(ChIP_gapSNVTMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, ChIP_gapSNVTMats[[x]])
  } else {
    ChIP_gapSNVTMats[[x]][[1]]
  }
}, mc.cores = length(ChIP_gapSNVTMats))

## ChIP
# gapSNVF
ChIP_gapSNVFMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr2", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENgapSNVF_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
# If gapSNVFs from multiple chromosomes are to be analysed,
# concatenate the corresponding gapSNVF coverage matrices
ChIP_gapSNVFMats <- mclapply(seq_along(ChIP_gapSNVFMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, ChIP_gapSNVFMats[[x]])
  } else {
    ChIP_gapSNVFMats[[x]][[1]]
  }
}, mc.cores = length(ChIP_gapSNVFMats))
...
ChIP_AthilaMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_soloLTRMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENsoloLTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
...
control_featureMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CEN180_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T, colClasses = c(rep("NULL", 1000/binSize), rep(NA, ((1000*2)+(180))/binSize), rep("NULL", 1000/binSize))))
  })
...
## control
# gapSNVT
control_gapSNVTMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENgapSNVT_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
# If gapSNVTs from multiple chromosomes are to be analysed,
# concatenate the corresponding gapSNVT coverage matrices
control_gapSNVTMats <- mclapply(seq_along(control_gapSNVTMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, control_gapSNVTMats[[x]])
  } else {
    control_gapSNVTMats[[x]][[1]]
  }
}, mc.cores = length(control_gapSNVTMats))

## control
# gapSNVF
control_gapSNVFMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr2", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENgapSNVF_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
# If gapSNVFs from multiple chromosomes are to be analysed,
# concatenate the corresponding gapSNVF coverage matrices
control_gapSNVFMats <- mclapply(seq_along(control_gapSNVFMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, control_gapSNVFMats[[x]])
  } else {
    control_gapSNVFMats[[x]][[1]]
  }
}, mc.cores = length(control_gapSNVFMats))
...
control_AthilaMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
...
control_soloLTRMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENsoloLTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
...
# gapSNVT
ChIPcontrol_gapSNVTMats <- mclapply(seq_along(ChIP_gapSNVTMats), function(x) {
  cbind( ( ( ChIP_gapSNVTMats[[x]][,1:(upstream/binSize)] * binSize ) + 1 ) / ( control_gapSNVTMats[[1]][,1:(upstream/binSize)] + 1 ),
         ChIP_gapSNVTMats[[x]][,((upstream/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize))],
         ( ( ChIP_gapSNVTMats[[x]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] * binSize ) + 1 ) /
             ( control_gapSNVTMats[[1]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] + 1 )
       )
}, mc.cores = length(ChIP_gapSNVTMats))
# gapSNVF
ChIPcontrol_gapSNVFMats <- mclapply(seq_along(ChIP_gapSNVFMats), function(x) {
  cbind( ( ( ChIP_gapSNVFMats[[x]][,1:(upstream/binSize)] * binSize ) + 1 ) / ( control_gapSNVFMats[[1]][,1:(upstream/binSize)] + 1 ),
         ChIP_gapSNVFMats[[x]][,((upstream/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize))],
         ( ( ChIP_gapSNVFMats[[x]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] * binSize ) + 1 ) /
             ( control_gapSNVFMats[[1]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] + 1 )
       )
}, mc.cores = length(ChIP_gapSNVFMats))
# Athila
...
for(x in seq_along(ChIPcontrol_featureMats)) {
  ...
  colnames(ChIPcontrol_ranLocMats[[x]]) <- c(paste0("u", 1:((upstream-1000)/binSize)),
                                             paste0("t", (((upstream-1000)/binSize)+1):(((upstream-1000)+bodyLength)/binSize)),
                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(ChIPcontrol_gapSNVTMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
  ...
  colnames(ChIPcontrol_gapSNVFMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
ChIPcontrol_mats <- mclapply(seq_along(ChIPcontrol_featureMats), function(x) {
  list(
       # features
       ChIPcontrol_featureMats[[x]],
       # ranLocs
       ChIPcontrol_ranLocMats[[x]],
       # gapSNVTs
       ChIPcontrol_gapSNVTMats[[x]],
       # gapSNVFs
       ChIPcontrol_gapSNVFMats[[x]],
       # Athilas
       ChIPcontrol_AthilaMats[[x]],
       # soloLTRs
       ChIPcontrol_soloLTRMats[[x]]
      ) 
}, mc.cores = length(ChIPcontrol_featureMats))

# Transpose matrix and convert into dataframe
# in which first column is window name
wideDFfeature_list_ChIPcontrol <- mclapply(seq_along(ChIPcontrol_mats), function(x) {
  lapply(seq_along(ChIPcontrol_mats[[x]]), function(y) {
    data.frame(window = colnames(ChIPcontrol_mats[[x]][[y]]),
               t(ChIPcontrol_mats[[x]][[y]]))
  })
}, mc.cores = length(ChIPcontrol_mats))

# Convert into tidy data.frame (long format)
tidyDFfeature_list_ChIPcontrol  <- mclapply(seq_along(wideDFfeature_list_ChIPcontrol), function(x) {
  lapply(seq_along(ChIPcontrol_mats[[x]]), function(y) {
    gather(data  = wideDFfeature_list_ChIPcontrol[[x]][[y]],
           key   = feature,
           value = coverage,
           -window)
  }) 
}, mc.cores = length(wideDFfeature_list_ChIPcontrol))

# Order levels of factor "window" so that sequential levels
# correspond to sequential windows
for(x in seq_along(tidyDFfeature_list_ChIPcontrol)) {
  for(y in seq_along(ChIPcontrol_mats[[x]])) {
    tidyDFfeature_list_ChIPcontrol[[x]][[y]]$window <- factor(tidyDFfeature_list_ChIPcontrol[[x]][[y]]$window,
                                                       levels = as.character(wideDFfeature_list_ChIPcontrol[[x]][[y]]$window))
  }
}
...
summaryDFfeature_list_ChIPcontrol  <- mclapply(seq_along(tidyDFfeature_list_ChIPcontrol), function(x) {
  lapply(seq_along(ChIPcontrol_mats[[x]]), function(y) {
    data.frame(window = as.character(wideDFfeature_list_ChIPcontrol[[x]][[y]]$window),
               n      = tapply(X     = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$coverage,
                               INDEX = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$window,
                               FUN   = length),
               mean   = tapply(X     = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$coverage,
                               INDEX = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$window,
                               FUN   = mean,
                               na.rm = TRUE),
               sd     = tapply(X     = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$coverage,
                               INDEX = tidyDFfeature_list_ChIPcontrol[[x]][[y]]$window,
                               FUN   = sd,
                               na.rm = TRUE))
  })
}, mc.cores = length(tidyDFfeature_list_ChIPcontrol))

for(x in seq_along(summaryDFfeature_list_ChIPcontrol)) {
  for(y in seq_along(ChIPcontrol_mats[[x]])) {
    summaryDFfeature_list_ChIPcontrol[[x]][[y]]$window <- factor(summaryDFfeature_list_ChIPcontrol[[x]][[y]]$window,
                                                          levels = as.character(wideDFfeature_list_ChIPcontrol[[x]][[y]]$window))
    summaryDFfeature_list_ChIPcontrol[[x]][[y]]$winNo <- factor(1:dim(summaryDFfeature_list_ChIPcontrol[[x]][[y]])[1])
    summaryDFfeature_list_ChIPcontrol[[x]][[y]]$sem <- summaryDFfeature_list_ChIPcontrol[[x]][[y]]$sd/sqrt(summaryDFfeature_list_ChIPcontrol[[x]][[y]]$n-1)
    summaryDFfeature_list_ChIPcontrol[[x]][[y]]$CI_lower <- summaryDFfeature_list_ChIPcontrol[[x]][[y]]$mean -
      qt(0.975, df = summaryDFfeature_list_ChIPcontrol[[x]][[y]]$n-1)*summaryDFfeature_list_ChIPcontrol[[x]][[y]]$sem
    summaryDFfeature_list_ChIPcontrol[[x]][[y]]$CI_upper <- summaryDFfeature_list_ChIPcontrol[[x]][[y]]$mean +
      qt(0.975, df = summaryDFfeature_list_ChIPcontrol[[x]][[y]]$n-1)*summaryDFfeature_list_ChIPcontrol[[x]][[y]]$sem
  }
}

# Convert list of lists summaryDFfeature_list_ChIPcontrol into
# a list of single data.frames containing all meta-profiles for plotting
featureTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[1]]
})
ranLocTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[2]]
})
gapSNVTTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[3]]
})
gapSNVFTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[4]]
})
AthilaTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[5]]
})
soloLTRTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[6]]
})
names(featureTmp) <- ChIPNamesPlot
names(ranLocTmp) <- ChIPNamesPlot
names(gapSNVTTmp) <- ChIPNamesPlot
names(gapSNVFTmp) <- ChIPNamesPlot
names(AthilaTmp) <- ChIPNamesPlot
names(soloLTRTmp) <- ChIPNamesPlot
summaryDFfeature_ChIPcontrol <- list(
  bind_rows(featureTmp, .id = "libName"),
  bind_rows(ranLocTmp, .id = "libName"),
  bind_rows(gapSNVTTmp, .id = "libName"),
  bind_rows(gapSNVFTmp, .id = "libName"),
  bind_rows(AthilaTmp, .id = "libName"),
  bind_rows(soloLTRTmp, .id = "libName")
)  
for(x in seq_along(summaryDFfeature_ChIPcontrol)) {
  summaryDFfeature_ChIPcontrol[[x]]$libName <- factor(summaryDFfeature_ChIPcontrol[[x]]$libName,
...
ymin_ChIPcontrol <- min(c(summaryDFfeature_ChIPcontrol[[1]]$CI_lower,
                          ...
                          summaryDFfeature_ChIPcontrol[[6]]$CI_lower))
ymax_ChIPcontrol <- max(c(summaryDFfeature_ChIPcontrol[[1]]$CI_upper,
                          ...
                          summaryDFfeature_ChIPcontrol[[6]]$CI_upper))
...
theme(
      ...
      plot.margin = unit(c(0.3,1.2,0.0,0.3), "cm"),
      plot.title = element_text(hjust = 0.5, size = 30)) +
ggtitle(bquote(.(gapSNVTNamePlot) ~ "(" * italic("n") ~ "=" ~
               .(prettyNum(summaryDFfeature$n[1],
                           big.mark = ",", trim = T)) *
               ")"))
...
theme(
      ...
      plot.margin = unit(c(0.3,1.2,0.0,0.3), "cm"),
      plot.title = element_text(hjust = 0.5, size = 30)) +
ggtitle(bquote(.(gapSNVFNamePlot) ~ "(" * italic("n") ~ "=" ~
               .(prettyNum(summaryDFfeature$n[1],
                           big.mark = ",", trim = T)) *
               ")"))
...
## soloLTR
summaryDFfeature <- summaryDFfeature_ChIPcontrol[[6]]
ggObj6_combined_ChIPcontrol <- ggplot(data = summaryDFfeature,
                                   mapping = aes(x = winNo,
                                                 y = mean,
                                                 group = libName)
                                  ) +
...
scale_x_discrete(breaks = c(1,
                            (upstream/Athila_binSize)+1,
                            (dim(summaryDFfeature_ChIPcontrol[[6]])[1]/length(ChIPNames))-(downstream/Athila_binSize),
                            dim(summaryDFfeature_ChIPcontrol[[6]])[1]/length(ChIPNames)),
                 labels = c(paste0("-", flankName),
                            featureStartLab,
                            featureEndLab,
                            paste0("+", flankName))) +
...
#              paste0(ChIPNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgapSNVT_CENgapSNVF_CENAthila_CENsoloLTR_in_t2t-col.20210610_",
              paste0(chrName, collapse = "_"), "_pseudocount1_ratios_per_feature_smoothed.pdf"),
...
for(x in seq_along(ChIP_featureMats)) {
  ...
  colnames(ChIP_ranLocMats[[x]]) <- c(paste0("u", 1:((upstream-1000)/binSize)),
                                             paste0("t", (((upstream-1000)/binSize)+1):(((upstream-1000)+bodyLength)/binSize)),
                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(ChIP_gapSNVTMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
  colnames(ChIP_gapSNVFMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
gapSNVTTmp <- lapply(seq_along(summaryDFfeature_list_ChIP), function(x) {
  summaryDFfeature_list_ChIP[[x]][[3]]
})
gapSNVFTmp <- lapply(seq_along(summaryDFfeature_list_ChIP), function(x) {
...
#              paste0(ChIPNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgapSNVT_CENgapSNVF_CENAthila_CENsoloLTR_in_t2t-col.20210610_",
              paste0(chrName, collapse = "_"), "_CEN180_variants_per_feature_smoothed.pdf"),
       plot = ggObjGA_combined,
       height = 6.5, width = 7*6, limitsize = FALSE)
...
for(x in seq_along(control_featureMats)) {
  ...
  colnames(control_ranLocMats[[x]]) <- c(paste0("u", 1:((upstream-1000)/binSize)),
                                             paste0("t", (((upstream-1000)/binSize)+1):(((upstream-1000)+bodyLength)/binSize)),
                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(control_gapSNVTMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
  colnames(control_gapSNVFMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
control_mats <- mclapply(seq_along(control_featureMats), function(x) {
  list(
       ...
       # gapSNVTs
       control_gapSNVTMats[[x]],
       # gapSNVFs
       control_gapSNVFMats[[x]],
       # Athilas
...
gapSNVTTmp <- lapply(seq_along(summaryDFfeature_list_control), function(x) {
  summaryDFfeature_list_control[[x]][[3]]
})
gapSNVFTmp <- lapply(seq_along(summaryDFfeature_list_control), function(x) {
...
names(gapSNVTTmp) <- controlNamesPlot
names(gapSNVFTmp) <- controlNamesPlot
...
#              paste0(controlNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgapSNVT_CENgapSNVF_CENAthila_CENsoloLTR_in_t2t-col.20210610_",
              paste0(chrName, collapse = "_"), "_CEN180_control_per_feature_smoothed.pdf"),
       plot = ggObjGA_combined,
       height = 6.5, width = 7*6, limitsize = FALSE)
