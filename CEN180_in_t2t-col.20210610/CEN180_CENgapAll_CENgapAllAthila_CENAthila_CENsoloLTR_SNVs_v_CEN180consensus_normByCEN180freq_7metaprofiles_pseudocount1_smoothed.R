...
# Usage:
# CEN180_CENgapAll_CENgapAllAthila_CENAthila_CENsoloLTR_SNVs_v_CEN180consensus_normByCEN180freq_7metaprofiles_pseudocount1_smoothed.R 'Chr1,Chr2,Chr3,Chr4,Chr5' 180 2000 2000 2kb 10 10 10bp 10bp '0.02,0.96'
...
if(length(chrName) == 5) {
  ...
  gapAllNamePlot <- "All CENgapAll"
  gapNamePlot <- "All CENgapAthila"
  ...
  gapAllNamePlot <- paste0(paste0(chrName, collapse = ","), " CENgapAll")
  gapNamePlot <- paste0(paste0(chrName, collapse = ","), " CENgapAthila")
...
ChIP_ranLocMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENgapAllAthila_in_",
...
## ChIP
# gapAll
ChIP_gapAllMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x],
                                ChIPNames[x],
                                "_CEN180_consensus_variants_MappedOn_t2t-col.20210610_around_CENgapAll_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(ChIPNames))
# If gapAlls from multiple chromosomes are to be analysed,
# concatenate the corresponding gapAll coverage matrices
ChIP_gapAllMats <- mclapply(seq_along(ChIP_gapAllMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, ChIP_gapAllMats[[x]])
  } else {
    ChIP_gapAllMats[[x]][[1]]
  }
}, mc.cores = length(ChIP_gapAllMats))
...
control_ranLocMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENgapAllAthila_in_",
                                chrName[y], "_CENranLoc_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
#                         header = T, colClasses = c(rep("NULL", 1000/binSize), rep(NA, ((1000*2)+(180))/binSize), rep("NULL", 1000/binSize))))
                         header = T))
  })
...
## control
# gapAll
control_gapAllMats <- mclapply(seq_along(controlNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(controlDirs[x],
                                controlNames[x],
                                "_MappedOn_t2t-col.20210610_around_CENgapAll_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = T))
  })
}, mc.cores = length(controlNames))
# If gapAlls from multiple chromosomes are to be analysed,
# concatenate the corresponding gapAll coverage matrices
control_gapAllMats <- mclapply(seq_along(control_gapAllMats), function(x) {
  if(length(chrName) > 1) {
    do.call(rbind, control_gapAllMats[[x]])
  } else {
    control_gapAllMats[[x]][[1]]
  }
}, mc.cores = length(control_gapAllMats))
...
# gapAll
ChIPcontrol_gapAllMats <- mclapply(seq_along(ChIP_gapAllMats), function(x) {
  cbind( ( ( ChIP_gapAllMats[[x]][,1:(upstream/binSize)] * binSize ) + 1 ) / ( control_gapAllMats[[1]][,1:(upstream/binSize)] + 1 ),
         ChIP_gapAllMats[[x]][,((upstream/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize))],
         ( ( ChIP_gapAllMats[[x]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] * binSize ) + 1 ) /
             ( control_gapAllMats[[1]][,((upstream/binSize)+(Athila_bodyLength/binSize)+1):((upstream/binSize)+(Athila_bodyLength/binSize)+(downstream/binSize))] + 1 )
       )
}, mc.cores = length(ChIP_gapAllMats))
# gap
...
#                                             paste0("t", (((upstream-1000)/binSize)+1):(((upstream-1000)+bodyLength)/binSize)),
#                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(ChIPcontrol_ranLocMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
  ...
  colnames(ChIPcontrol_gapAllMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
ChIPcontrol_mats <- mclapply(seq_along(ChIPcontrol_featureMats), function(x) {
  list(
       ...
       # gapAlls
       ChIPcontrol_gapAllMats[[x]],
       # gaps
       ChIPcontrol_gapMats[[x]],
       # Athilas
...
gapAllTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
  summaryDFfeature_list_ChIPcontrol[[x]][[3]]
})
gapTmp <- lapply(seq_along(summaryDFfeature_list_ChIPcontrol), function(x) {
...
summaryDFfeature_ChIPcontrol <- list(
  ...
  bind_rows(gapAllTmp, .id = "libName"),
...
scale_x_discrete(breaks = c(1,
                            ...
                            (dim(summaryDFfeature_ChIPcontrol[[2]])[1]/length(ChIPNames))-(downstream/Athila_binSize),
                            dim(summaryDFfeature_ChIPcontrol[[2]])[1]/length(ChIPNames)),
...
theme(
      ...
      plot.margin = unit(c(0.3,1.2,0.0,0.3), "cm"),
      plot.title = element_text(hjust = 0.5, size = 30)) +
ggtitle(bquote(.(gapAllNamePlot) ~ "(" * italic("n") ~ "=" ~
               .(prettyNum(summaryDFfeature$n[1],
                           big.mark = ",", trim = T)) *
               ")"))
...
#              paste0(ChIPNames, collapse = "_"),
              "_avgProfiles_around",
              "_CEN180_CENranLoc_CENgapAll_CENgapAllAthila_CENAthila_CENsoloLTR_in_t2t-col.20210610_",
...
#                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(ChIP_ranLocMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                      paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                      paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
  colnames(ChIP_gapAllMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
gapAllTmp <- lapply(seq_along(summaryDFfeature_list_ChIP), function(x) {
...
#                                             paste0("d", ((((upstream-1000)+bodyLength)/binSize)+1):((((upstream-1000)+bodyLength)/binSize)+((downstream-1000)/binSize))))
  colnames(control_ranLocMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                         paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                         paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
  colnames(control_gapAllMats[[x]]) <- c(paste0("u", 1:(upstream/Athila_binSize)),
                                          paste0("t", ((upstream/Athila_binSize)+1):((upstream+Athila_bodyLength)/Athila_binSize)),
                                          paste0("d", (((upstream+Athila_bodyLength)/Athila_binSize)+1):(((upstream+Athila_bodyLength)/Athila_binSize)+(downstream/Athila_binSize))))
...
gapAllTmp <- lapply(seq_along(summaryDFfeature_list_control), function(x) {
...
names(gapAllTmp) <- controlNamesPlot
...
scale_x_discrete(breaks = c(1,
                            (upstream/Athila_binSize)+1,
                            (dim(summaryDFfeature_control[[2]])[1]/length(controlNames))-(downstream/Athila_binSize),
                            dim(summaryDFfeature_control[[2]])[1]/length(controlNames)),
                 labels = c(paste0("-", flankName),
                            featureStartLab,
                            featureEndLab,
