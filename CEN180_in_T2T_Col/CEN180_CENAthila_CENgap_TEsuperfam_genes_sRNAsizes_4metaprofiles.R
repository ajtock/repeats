...
# Usage:
# /applications/R/R-4.0.0/bin/Rscript CEN180_CENAthila_CENgap_TEsuperfam_genes_sRNAsizes_4metaprofiles.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 180 11000 1000 2200 1000 1kb 10 200 10bp 10bp '0.02,0.96' 'Col_0_sRNA_SRR1042171,ddm1_2_sRNA_SRR1042172,s7_Col_0_sRNA_SRR1042176,s8_ddm1_2_sRNA_SRR1042177' 'sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col' 'wt (TruSeq),ddm1-2 (TruSeq),wt (NEBNext),ddm1-2 (NEBNext)' 'grey20,magenta,grey40,magenta4' '18'
...
#                                        split = ",")))
#ChIPNames <- unlist(strsplit("Col_0_sRNA_SRR1042171,ddm1_2_sRNA_SRR1042172,s7_Col_0_sRNA_SRR1042176,s8_ddm1_2_sRNA_SRR1042177",
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col,sRNAseq_floral_Creasey_Martienssen_2014_Nature/snakemake_sRNAseq_T2T_Col",
#                                split = ","))
#ChIPNamesPlot <- unlist(strsplit("wt (TruSeq),ddm1-2 (TruSeq),wt (NEBNext), ddm1-2 (NEBNext)",
#                                 split = ","))
#ChIPColours <- unlist(strsplit("grey20,magenta,grey40,magenta4",
#                               split = ","))
#sRNAsize <- "18"
...
ChIPNamesDir <- unlist(strsplit(args[15],
                                split = ","))
ChIPNamesPlot <- unlist(strsplit(args[16],
                                 split = ","))
ChIPColours <- unlist(strsplit(args[17],
                               split = ","))
sRNAsize <- as.character(args[18])
yLabPlot <- paste0(substring(sRNAsize, first = 1, last = 2),
                   "-nt sRNAs (TPM)")
...
ChIP_featureMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_", sRNAsize, "nt_sort_norm_CEN180_in_",
...
ChIP_gapMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_", sRNAsize, "nt_sort_norm_CENgap_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_AthilaMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CEN180profiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_", sRNAsize, "nt_sort_norm_CENAthila_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_TEsfMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "TEprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_", sRNAsize, "nt_sort_norm_TEs_Gypsy_LTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_geneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "geneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_", sRNAsize, "nt_sort_norm_genes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
