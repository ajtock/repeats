...
# (windowed variant sums / windowed CEN180 sums)
  ...
  lapply(seq_along(ChIP_mats[[x]]), function(y) {
    data.frame(window = as.character(wideDFfeature_list_ChIP[[x]][[y]]$window),
               n      = tapply(X     = tidyDFfeature_list_ChIP[[x]][[y]]$coverage,
                               ...
                               FUN   = length),
               sum    = tapply(X     = tidyDFfeature_list_ChIP[[x]][[y]]$coverage,
                               INDEX = tidyDFfeature_list_ChIP[[x]][[y]]$window,
                               FUN   = sum,
                               na.rm = TRUE),
  ...
  lapply(seq_along(control_mats[[x]]), function(y) {
    data.frame(window = as.character(wideDFfeature_list_control[[x]][[y]]$window),
               n      = tapply(X     = tidyDFfeature_list_control[[x]][[y]]$coverage,
                               ...
                               FUN   = length),
               sum    = tapply(X     = tidyDFfeature_list_control[[x]][[y]]$coverage,
                               INDEX = tidyDFfeature_list_control[[x]][[y]]$window,
                               FUN   = sum,
                               na.rm = TRUE),
...
for(x in seq_along(summaryDFfeature_ChIPcontrol)) {
  summaryDFfeature_ChIPcontrol[[x]]$sum <- (summaryDFfeature_ChIP[[x]]$sum + 0) / (summaryDFfeature_control[[x]]$sum + 0)
  summaryDFfeature_ChIPcontrol[[x]]$mean <- (summaryDFfeature_ChIP[[x]]$mean + 0) / (summaryDFfeature_control[[x]]$mean + 0)
  summaryDFfeature_ChIPcontrol[[x]]$sd <- (summaryDFfeature_ChIP[[x]]$sd + 0) / (summaryDFfeature_control[[x]]$sd + 0)
  summaryDFfeature_ChIPcontrol[[x]]$sem <- (summaryDFfeature_ChIP[[x]]$sem + 0) / (summaryDFfeature_control[[x]]$sem + 0)
  summaryDFfeature_ChIPcontrol[[x]]$CI_lower <- (summaryDFfeature_ChIP[[x]]$CI_lower + 0) / (summaryDFfeature_control[[x]]$CI_lower + 0)
  summaryDFfeature_ChIPcontrol[[x]]$CI_upper <- (summaryDFfeature_ChIP[[x]]$CI_upper + 0) / (summaryDFfeature_control[[x]]$CI_upper + 0)
}
 

# Define y-axis limits
ymin_ChIPcontrol <- min(c(summaryDFfeature_ChIPcontrol[[1]]$sum,
                   summaryDFfeature_ChIPcontrol[[2]]$sum,
                   summaryDFfeature_ChIPcontrol[[3]]$sum,
                   summaryDFfeature_ChIPcontrol[[4]]$sum,
                   summaryDFfeature_ChIPcontrol[[5]]$sum))
ymax_ChIPcontrol <- max(c(summaryDFfeature_ChIPcontrol[[1]]$sum,
                   summaryDFfeature_ChIPcontrol[[2]]$sum,
                   summaryDFfeature_ChIPcontrol[[3]]$sum,
                   summaryDFfeature_ChIPcontrol[[4]]$sum,
                   summaryDFfeature_ChIPcontrol[[5]]$sum))
...
#              paste0(ChIPNames, collapse = "_"),
              ...
              paste0(chrName, collapse = "_"), "_sum_ratios_all_features.pdf"),
