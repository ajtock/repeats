...
if(length(chrName) == 5) {
  ...
  gapNamePlot <- "All CENAthila"
  AthilaNamePlot <- paste0("All CEN", AthilaFam, "Fam")
  ...
  gapNamePlot <- paste0(paste0(chrName, collapse = ","), " CENAthila")
  AthilaNamePlot <- paste0(paste0(chrName, collapse = ","), " CEN", AthilaFam, "Fam")
  TEsfNamePlot <- paste0(paste0(chrName, collapse = ","), " nonCENGypsy")
  soloLTRNamePlot <- paste0(paste0(chrName, collapse = ","), " CENsoloLTR")
}

# Get row indices corresponding to AthilaFam elements,
# to be used for extracting rows from coverage matrices
CENAthila_bed <- read.table(paste0("/home/ajt200/analysis/nanopore/T2T_Col/annotation/CENAthila/",
                                   "CENAthila_in_T2T_Col_", paste0(chrName, collapse = "_"), ".bed"),
                            header = F)
colnames(CENAthila_bed) <- c("chr", "start0based", "end", "name", "class", "strand")
AthilaFam_rowIndices <- which(CENAthila_bed$class == AthilaFam)
...
ChIP_AthilaMats <- mclapply(seq_along(ChIP_AthilaMats), function(x) {
  ChIP_AthilaMats[[x]][c(AthilaFam_rowIndices), , drop = FALSE]
...
ChIP_TEsfMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "TEprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", context, "_nonCEN_TEs_Gypsy_LTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_soloLTRMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(which(chrName %in% c("Chr1", "Chr4", "Chr5")), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "CENsoloLTRprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", context, "_CENsoloLTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
