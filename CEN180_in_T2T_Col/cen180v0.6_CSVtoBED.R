...
# Usage:
# ./cen180v0.6_CSVtoBED.R 'Chr1,Chr2,Chr3,Chr4,Chr5'
...
tab <- read.csv("cen180v0.6.csv", header = T)
colnames(tab)[1] <- "index"
tab1 <- read.csv("cen180v0.3.csv", header = T)
stopifnot(identical(tab$chromosome, tab1$chromosome))
stopifnot(identical(tab$start, tab1$start))
stopifnot(identical(tab$end, tab1$end))
stopifnot(identical(tab$strand, tab1$strand))
stopifnot(identical(tab$seq, tab1$seq))
stopifnot(identical(tab$weightedSNV, tab1$weightedSNV))

# In tab relative to tab1, Piotr has amended HORlengthsSum and HORcount values, and added 2 additional metrics
# "that count how many times a repeat of the same sequence is present elsewhere in the chromosome or genome"
# (column names "repeated.chromosome" and "repeated.genome", renamed below as "chromosomeRepeats" and "genomeRepeats")
print(identical(tab$HORlengthsSum, tab1$HORlengthsSum))
#[1] FALSE
print(identical(tab$HORcount, tab1$HORcount))
#[1] FALSE
colnames(tab)[which(colnames(tab) == "repeated.chromosome")] <- "repeatsChromosome"
colnames(tab)[which(colnames(tab) == "repeated.genome")] <- "repeatsGenome"
...
CEN180GR <- GRanges(seqnames = tab$chromosome,
                    ...
                    repeatsChromosome = tab$repeatsChromosome,
                    repeatsGenome = tab$repeatsGenome)
...
CEN180GR <- sortSeqlevels(CEN180GR)
...
CEN180_bed <- data.frame(chr = as.character(seqnames(CEN180GR)),
                         ...
                         repeatsChromosome = CEN180GR$repeatsChromosome,
                         repeatsGenome = CEN180GR$repeatsGenome)
