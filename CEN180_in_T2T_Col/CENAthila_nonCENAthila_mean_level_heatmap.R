...
# Usage:
# /applications/R/R-4.0.0/bin/Rscript CENAthila_nonCENAthila_mean_level_heatmap.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 180 2000 2000 2000 '2kb' 10 10 10bp 10bp
...
tab1 <- read.csv("/home/ajt200/analysis/nanopore/T2T_Col/annotation/TEs/t2t.largegaps--Athilafinal.csv",
                 header = T, na.strings = "na")
colnames(tab1)[1] <- "chr"
tab1$chr <- gsub(pattern = "^", replacement = "Chr",
                 x = tab1$chr)
...
tab1$phylo <- tab1$class2
tab1$phylo[grep("_v2", tab1$phylo)] <- "Athila6B"
tab1$phylo[grep("6A", tab1$phylo)] <- "Athila6A"
tab1$phylo <- toupper(tab1$phylo)
#tab1$phylo <- gsub("ATHILA1", "ATHILA", tab1$phylo)
tab1$phylo <- gsub("^", "CEN", tab1$phylo)
...
reject_rowIndices <- which(is.na(tab1$genome_5LTR_start))
# Remove rejected elements
CENATHILA <- tab1[-reject_rowIndices,]
# Retain only columns that are in nonCENATHILA table
CENATHILA <- data.frame(chr = CENATHILA$chr,
                        TE_ID = CENATHILA$TE_ID,
                        number = CENATHILA$number,
                        TSDleft = CENATHILA$left_TSD,
                        TSDright = CENATHILA$right_TSD,
                        TSD = CENATHILA$TSD,
                        genome_left_coord_FL = CENATHILA$genome_left_coord_FL,
                        genome_right_coord_FL = CENATHILA$genome_right_coord_FL,
                        leftLTR_start = CENATHILA$genome_5LTR_start,
                        leftLTR_stop = CENATHILA$genome_5LTR_end,
                        rightLTR_start = CENATHILA$genome_3LTR_start,
                        rightLTR_stop = CENATHILA$genome_3LTR_end,
                        leftLTR.length = CENATHILA$length_5LTR,
                        internal.length = abs(CENATHILA$genome_3LTR_start-CENATHILA$genome_5LTR_end+1),
                        rightLTR.length = CENATHILA$length_3LTR,
                        fulllength = CENATHILA$genome_right_coord_FL-CENATHILA$genome_left_coord_FL+1,
                        phylo = CENATHILA$phylo,
                        direction = CENATHILA$direction,
                        comment = CENATHILA$comment)
                        
# Load table of non-centromeric Athila
tab2 <- read.table("/home/ajt200/analysis/nanopore/T2T_Col/annotation/TEs/Athilas_nonCentromeric.bin",
                  header = T, sep = "\t")
colnames(tab2)[1] <- "chr"
tab2$phylo <- toupper(tab2$phylo)
tab2$phylo <- gsub("^", "nonCEN", tab2$phylo)
nonCENATHILA <- tab2
...
pdf(paste0(plotDir,
           ...
           paste0(chrName, collapse = "_"), "_mean_RNAseq_sRNAseq_DNAmeth_heatmap_colourQuantiles0.5to0.95.pdf"),
...
write.table(tab_extend,
            file = paste0("/home/ajt200/analysis/nanopore/T2T_Col/annotation/TEs/",
                          "t2t_ATHILA_with_mean_RNA_sRNA_DNAmeth_v230421.tsv"),
