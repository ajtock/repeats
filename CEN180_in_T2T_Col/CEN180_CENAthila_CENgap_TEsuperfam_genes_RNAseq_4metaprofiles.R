...
# Calculate and plot metaprofiles of RNA-seq
# (CEN180 windowed means and 95% confidence intervals, CIs)
# for all CEN180 sequences and randomly positioned loci

# Usage:
# /applications/R/R-4.0.0/bin/Rscript CEN180_CENAthila_CENgap_TEsuperfam_genes_RNAseq_4metaprofiles.R 'Chr1,Chr2,Chr3,Chr4,Chr5' both 180 11000 1000 2200 1000 1kb 10 200 10bp 10bp '0.02,0.96' 'WT_RNAseq_Chris_Rep1,WT_RNAseq_Chris_Rep2,kss_RNAseq_Chris_Rep1,kss_RNAseq_Chris_Rep2' '170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col' 'wt Rep1,wt Rep2,kss Rep1,kss Rep2' 'grey20,grey40,darkorange,darkorange4'
...
#                                        split = ",")))
#ChIPNames <- unlist(strsplit("WT_RNAseq_Chris_Rep1,WT_RNAseq_Chris_Rep2,kss_RNAseq_Chris_Rep1,kss_RNAseq_Chris_Rep2",
#                             split = ","))
#ChIPNamesDir <- unlist(strsplit("170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col,170918_Chris_RNAseq_Col_kss/fastq_pooled/snakemake_RNAseq_STAR_T2T_Col",
#                                split = ","))
#ChIPNamesPlot <- unlist(strsplit("wt Rep1,wt Rep2,kss Rep1,kss Rep2",
#                                 split = ","))
#ChIPColours <- unlist(strsplit("grey20,grey40,darkorange,darkorange4",
#                               split = ","))
...
ChIP_TEsfMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "TEprofiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_sort_norm_TEs_Gypsy_LTR_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
...
ChIP_geneMats <- mclapply(seq_along(ChIPNames), function(x) {
  lapply(seq_along(chrName), function(y) {
    as.matrix(read.table(paste0(ChIPDirs[x], "geneProfiles/matrices/",
                                ChIPNames[x],
                                "_MappedOn_T2T_Col_", align, "_sort_norm_genes_in_",
                                chrName[y], "_matrix_bin", binSize, "bp_flank", flankName, ".tab"),
                         header = F, skip = 3))
  })
}, mc.cores = length(ChIPNames))
