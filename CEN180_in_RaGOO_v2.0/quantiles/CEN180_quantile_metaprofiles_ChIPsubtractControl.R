...
# Usage:
# /applications/R/R-3.5.0/bin/Rscript CEN180_quantile_metaprofiles_ChIPsubtractControl.R 'Chr1,Chr2,Chr3,Chr4,Chr5' wSNV 4 both 180 1000 1kb 10 '0.02,0.30'
...
ChIPNamesDir <- c(
                  ...
                  rep("histones_10dps_seedling_Berger_2020/snakemake_ChIPseq_RaGOO_v2.0", 12)
                 )
log2ChIPNamesPlot <- c(
                       "CENH3",
                       "LoCENH3",
...
controlNamesDir <- c(
                     ...
                     rep("histones_10dps_seedling_Berger_2020/snakemake_ChIPseq_RaGOO_v2.0", 3)
                    )
controlNamesPlot <- c(
...
log2ChIP_featureMats <- mclapply(seq_along(ChIP_featureMats), function(x) {
  if ( grepl("MNase", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[2], " for MNase - gDNA calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[2]]
  } else if ( grepl("SPO11oligos", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[3], " for SPO11-1-oligos - gDNA calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[3]]
  } else if ( grepl("set1", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[4], " for ChIP - input calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[4]]
  } else if ( grepl("set2", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[5], " for ChIP - input calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[5]]
#  } else if ( grepl("set3", ChIPNames[x]) ) {
#    print(paste0(ChIPNames[x], " library; using ", controlNames[6], " for ChIP - input calculation"))
#    ChIP_featureMats[[x]] - control_featureMats[[6]]
  } else if ( grepl("set5", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[6], " for ChIP - input calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[6]]
#  } else if ( grepl("set7", ChIPNames[x]) ) {
#    print(paste0(ChIPNames[x], " library; using ", controlNames[8], " for ChIP - input calculation"))
#    ChIP_featureMats[[x]] - control_featureMats[[8]]
  } else {
    print(paste0(ChIPNames[x], " library; using ", controlNames[1], " for ChIP - input calculation"))
    ChIP_featureMats[[x]] - control_featureMats[[1]]
  }
}, mc.cores = length(ChIP_featureMats))
...
log2ChIP_ranLocMats <- mclapply(seq_along(ChIP_ranLocMats), function(x) {
  if ( grepl("MNase", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[2], " for MNase - gDNA calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[2]]
  } else if ( grepl("SPO11oligos", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[3], " for SPO11-1-oligos - gDNA calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[3]]
  } else if ( grepl("set1", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[4], " for ChIP - input calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[4]]
  } else if ( grepl("set2", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[5], " for ChIP - input calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[5]]
#  } else if ( grepl("set3", ChIPNames[x]) ) {
#    print(paste0(ChIPNames[x], " library; using ", controlNames[6], " for ChIP - input calculation"))
#    ChIP_ranLocMats[[x]] - control_ranLocMats[[6]]
  } else if ( grepl("set5", ChIPNames[x]) ) {
    print(paste0(ChIPNames[x], " library; using ", controlNames[6], " for ChIP - input calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[6]]
#  } else if ( grepl("set7", ChIPNames[x]) ) {
#    print(paste0(ChIPNames[x], " library; using ", controlNames[8], " for ChIP - input calculation"))
#    ChIP_ranLocMats[[x]] - control_ranLocMats[[8]]
  } else {
    print(paste0(ChIPNames[x], " library; using ", controlNames[1], " for ChIP - input calculation"))
    ChIP_ranLocMats[[x]] - control_ranLocMats[[1]]
  }
}, mc.cores = length(ChIP_ranLocMats))
...
ggsave(paste0(plotDir,
              "ChIPsubtractControl_avgProfiles_around_", quantiles, "quantiles",
               "_by_", orderingFactor,
               "_of_CEN180_in_RaGOO_v2.0_",
